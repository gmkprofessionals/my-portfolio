...
import { BlogType } from '../route';
import Blogs from '../../../../../Modals/Blogs';
import dbConnect from '../../../../../dbConnect';

export async function GET({ params }: { params: Promise<{ BlgId: string }>}){
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ BlgId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, msg: `Blog ${action} successfully.`, updatedBlog}, { status: 200 });

  } catch (error: any) {
    return NextResponse.json({ success: false, msg: 'Error updating publish status.', error: error.message }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest,{ params }: { params: Promise<{ BlgId: string }>}) {
    ...
    try {
        ...
        if (!BlgId) {
            ...
            return NextResponse.json({delBlg, success:true, msg: "Blog deleted successfully." }, { status: 200 });
        }
    } catch (error:any) {
        if (error.name === 'ValidationError') {
            const messages = Object.values(error.errors).map((val:any) => val.message);
